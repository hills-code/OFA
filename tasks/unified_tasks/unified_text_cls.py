...
@dataclass
class UnifiedTextClsConfig(OFAConfig):
    ans2label_dict: Optional[str] = field(
        default='none',
        metadata={"help": 'answer to label dict pickle file path'},
    )
    task_name: Optional[str] = field(
        default='none',
        metadata={"help": 'the name of specific text classification task'},
    )
...
@register_task("unified_text_cls", dataclass=UnifiedTextClsConfig)
class UnifiedTextClsTask(OFATask):
    def __init__(self, cfg: UnifiedTextClsConfig, src_dict, tgt_dict):
        super().__init__(cfg, src_dict, tgt_dict)
        with open(self.cfg.ans2label_dict, 'rb') as fp:
            self.ans2label_dict = pickle.load(fp)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = UnifiedTextClsDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
